...
class FileManager:
    ...
    def recalculate_total_files_size(self):
        self.totalsize = functools.reduce(lambda x, y: x + y, list(
    ...
    def recalculate_selected_files_size(self):
        self.selectedsize = functools.reduce(lambda x, y: x + y, list(
            map(lambda i: os.path.getsize(os.path.join(self.wd, self.files[i])),
                self.selectedfiles))) if self.selectedfiles else 0
...
class FileFunctions(urwid.WidgetPlaceholder):
    ...
    def metamapper(self, x):
        return urwid.AttrMap(SelectableText(FileMetadata(self.fm.wd, x).render(self.fm.maxfilenamelen)),
    ...
    def __init__(self, wd):
        ...
        self.column_titles = urwid.AttrWrap(
            urwid.Text('Name\t      Size  Last Modified'.expandtabs(self.fm.maxfilenamelen + 2)), 'columns')
    ...
    def update_statbox(self):
        stat_listed = urwid.Text(
            '{:>24}'.format(f'{self.fm.filecount()} files LISTed   = ') + f'{self.render_size(self.fm.totalsize)}',
            'left')
        stat_selected = urwid.Text('{:>24}'.format(
            f'{self.fm.selectedcount()} files SELECTed = ') + f'{self.render_size(self.fm.selectedsize)}', 'left')
        stat_subdir = urwid.Text(
            '{:>24}'.format(f'{self.fm.filecount()} files in sub-dir = ') + f'{self.render_size(self.fm.totalsize)}',
            'left')
    ...
    def select(self):
        ...
        self.listwalker[idx] = urwid.AttrMap(
            SelectableText(FileMetadata(self.fm.wd, self.fm.files[idx]).render(self.fm.maxfilenamelen)),
